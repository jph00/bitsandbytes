...
def create_dynamic_map(signed=True, max_exponent_bits=7, total_bits=8):
    ...
    if signed == False and total_bits == 4:
        data = data[:-1]
...
def create_custom_map(seed=0, scale=0.01, isgptq=False):
    #v = [12, 10, 8, 6, 3, 2, 1]
    ...
    #v = v + [0] + v
    if isgptq:
        v = get_4bit_type('nf4').tolist()
        v = np.array(v)
        np.random.seed(11)
        v += np.random.randn(16)*0.01
        v = v.tolist()
